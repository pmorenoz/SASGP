...
class SASGP(nn.Module):
    ...
    """
    def __init__(self, kernel, likelihood, data_dim=None, latent_dim=None, data_size=None,
                 learning_rate=1e-2, active_set=20, device="cpu"):
        ...
        # Amortization Net
        self.amortization_net = nn.Sequential(nn.Linear(self.data_dim, 512),
                                              nn.ReLU(),
                                              nn.Linear(512, 256),
                                              nn.ReLU(),
    ...
    def nlpd(self, x_star, x):
        ...
        nlpd = torch.abs(0.5*math.log(2*math.pi) + 0.5*torch.mean(torch.log(var_star) + (x_fold - mu_star)**2/var_star))
        return nlpd
...
class BayesianSASGP(nn.Module):
    ...
    """
    def __init__(self, kernel, likelihood, data_dim=None, latent_dim=None, data_size=None,
                 learning_rate=1e-2, active_set=20, device="cpu", small_amortization=True):
        ...
        if small_amortization:
            self.mu_z = nn.Sequential(nn.Linear(self.data_dim, 50),
                                      nn.Tanh(),
                                      nn.Linear(50, self.latent_dim))

            self.var_z = nn.Sequential(nn.Linear(self.data_dim, 50),
                                       nn.Tanh(),
                                       nn.Linear(50, self.latent_dim),
                                       nn.Softplus())
        else:
            self.mu_z = nn.Sequential(nn.Linear(self.data_dim, 512),
                                                  nn.ReLU(),
                                                  nn.Linear(512, 256),
                                                  nn.ReLU(),
                                                  nn.Linear(256, self.latent_dim))

            self.var_z = nn.Sequential(nn.Linear(self.data_dim, 512),
                                                  nn.ReLU(),
                                                  nn.Linear(512, 256),
                                                  nn.ReLU(),
                                                  nn.Linear(256, self.latent_dim),
    ...
    def forward(self, x):
        ...
        if x_rest.shape[0] > 0:
            ...
            # Expectation term
            za_sample = mu_za + torch.randn_like(var_za) * var_za.sqrt()  # this computation could be saved
            Kaa = self.kernel.K(za_sample, za_sample) + torch.exp(self.likelihood.log_sigma) * torch.eye(za_sample.shape[0])
